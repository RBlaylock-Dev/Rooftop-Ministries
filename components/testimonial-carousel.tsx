...
const testimonials = [
  {
    id: 1,
    quote:
      "The life coaching sessions with Pastor Marissa completely changed my perspective on relationships and purpose. I finally feel like I'm walking in my calling.",
    name: "Sarah Johnson",
  ...
  return (
    <div className="relative max-w-4xl mx-auto">
      ...
      <Button
        variant="outline"
        size="icon"
        className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 w-12 h-12 rounded-full bg-white shadow-lg border-2 hover:bg-slate-50"
      ...
      <Button
        variant="outline"
        size="icon"
        className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 w-12 h-12 rounded-full bg-white shadow-lg border-2 hover:bg-slate-50"
        onClick={goToNext}
        ...
        {testimonials.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-colors duration-200 ${
              index === currentIndex ? "bg-primary-600" : "bg-slate-300 hover:bg-slate-400"
            }`}
